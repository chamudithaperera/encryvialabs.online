    ...
    >
      <div className="container flex h-20 items-center justify-between gap-4">
        <Link href="/#home" className="group flex items-center gap-3" onClick={() => setOpen(false)}>
          <span className="grid h-10 w-10 place-items-center rounded-2xl bg-gradient-to-br from-blue-600 to-cyan-500 text-white shadow-lg shadow-blue-500/20 transition-transform group-hover:scale-105">
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {navLinks.map((item) => (
            <button
              ...
              className="rounded-xl px-4 py-2 text-sm font-medium text-slate-600 transition-all hover:bg-blue-50 hover:text-blue-600"
            >
              {item.label}
            </button>
          ))}
          ...
          <div className="container animate-fade-in space-y-2 py-4">
            {navLinks.map((item) => (
              <button
                ...
                type="button"
                onClick={() => handleNavClick(item.href)}
                className="block w-full rounded-2xl px-4 py-3 text-left text-sm font-medium text-slate-600 transition-all hover:bg-blue-50 hover:text-blue-600"
              >
                {item.label}
              </button>
            ))}
            <div className="pt-2">
              <Button href="/#contact" className="w-full" onClick={() => handleNavClick("/#contact")}>
                Get Started
              </Button>
            </div>
          </div>
